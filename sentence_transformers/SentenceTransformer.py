...
class SentenceTransformer(nn.Sequential):
    ...
    def fit(self,
        ...
        for loss_model in loss_models:
            ...
            if optimizer_custom is not None:

                optimizers.append(optimizer_custom)
            else:
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            for _ in trange(steps_per_epoch, desc="Iteration", smoothing=0.05, disable=not show_progress_bar):
                ...
                # print(f'\nloss value : {loss_value} / step {training_steps}')
                self.loss_step.append(loss_value.item())
        end=time.time()
        print(f'total traing time {(end-start)/60} ')
    ...
    def _eval_during_training(self, evaluator, output_path, save_best_model, epoch, steps, callback):
        ...
        if evaluator is not None:
            ...
            if score > self.best_score:
                ...
                if save_best_model:
                    self.save(output_path)
        with open(output_path+'/loss_per_step.csv', 'w', newline='') as csv_file:
            csv_writer = csv.writer(csv_file)
            csv_writer.writerows(map(lambda x: [x], self.loss_step))
            print(output_path)
