...
import os
from peft import get_peft_config,  get_peft_model,PromptEncoderConfig
...
class PeftTransformer(nn.Module):
    ...
    def __init__(self, model_name_or_path: str, max_seq_length: Optional[int] = None,
        ...
        super(PeftTransformer, self).__init__()
        self.config_keys = ['max_seq_length', 'do_lower_case']
        self.do_lower_case = do_lower_case

        config = AutoConfig.from_pretrained(model_name_or_path, **model_args, cache_dir=cache_dir)
        self._load_model(model_name_or_path, config,peft_config, cache_dir)
    ...
    def _load_model(self, model_name_or_path, config,peft_config, cache_dir):
        ...
        if isinstance(config, T5Config):
            ...
            # print(peft_config)
            peft_config = PromptEncoderConfig(task_type="TOKEN_CLS", num_virtual_tokens=20, encoder_hidden_size=768,num_layers=5,token_dim=768,num_attention_heads=4)
            print(peft_config)
          
            self.auto_model=get_peft_model(model,peft_config)
            print("peft model")
    ...
    def load(input_path: str):
        ...
        return PeftTransformer(model_name_or_path=input_path, **config)
