...
class InformationRetrievalEvaluator(SentenceEvaluator):
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1, *args, **kwargs) -> float:
        ...
        if self.asym==True:
            scores=self.compute_metrice_asym(model,*args,**kwargs)
        else:
    ...
    def compute_metrice_asym(self, model, corpus_model = None, corpus_embeddings: Tensor = None) -> Dict[str, float]:
        max_k = max(max(self.mrr_at_k), max(self.ndcg_at_k), max(self.accuracy_at_k), max(self.precision_recall_at_k), max(self.map_at_k))

        # Compute embedding for the queries
        query_embeddings = model.encode({'QRY': self.queries}, show_progress_bar=self.show_progress_bar, batch_size=self.batch_size, convert_to_tensor=True)
        ...
        for corpus_start_idx in trange(0, len(self.corpus), self.corpus_chunk_size, desc='Corpus Chunks', disable=not self.show_progress_bar):
            ...
            #Encode chunk of corpus
            if corpus_embeddings is None:
                sub_corpus_embeddings = corpus_model.encode({'DOC':self.corpus[corpus_start_idx:corpus_end_idx]}, show_progress_bar=False, batch_size=self.batch_size, convert_to_tensor=True)
            else:
                sub_corpus_embeddings = corpus_embeddings[corpus_start_idx:corpus_end_idx]
        ...
        #Compute scores
        scores = {name: self.compute_metrics_asym(queries_result_list[name]) for name in self.score_functions}
